...
def vowel_to_bar(text: str) -> str:
    ...
    def replace_func(match):
        ...
        vowel = char_to_vowel(first)
        res = match.group(0)
        if vowel == match.group(0)[1]:
            res = first + "ー"
        elif vowel == "エ" and match.group(0)[1] == "イ":
            res = first + "ー"
        elif vowel == "オ" and match.group(0)[1] == "ウ":
            res = first + "ー"
        return res
...
class KanaPattern:
    ...
    def get_patterns() -> dict[str, str]:
        ...
        kana_multi = f"({kana_u}[ァィェォ]|{kana_td}[ャィュョ]|{kana_i}[ャュョ]|{kana_i2}ェ|[トド]ゥ)"
...
def small_vowel_to_large(text: str) -> str:
    ...
    Returns:
        変換後のテキスト
    """
    s2l = {"ァ": "ア", "ィ": "イ", "ゥ": "ウ", "ェ": "エ", "ォ": "オ", 
           "ヮ": "ワ", "ャ": "ヤ", "ュ": "ユ", "ョ": "ヨ"}
    
    # 先頭以外の置換
    def replace_func(match):
        match_text = match.group(0)
        if re.match(r'[ウクスツヌフムユルグズヅブプヴ][ァヮォ]', match_text):  # ウ段の後ろのァヮェォ
            return match_text
        elif re.match(r'[トド]ゥ', match_text):
            return match_text
        elif re.match(r'[キシチニヒミリギジヂビピテデ][ャュョ]', match_text):  # イ段の後ろのャュョ
            return match_text
        elif re.match(r'[ウクスツヌフムユルグズヅブプヴテデ]ィ', match_text):  # テデの後ろのィ
            return match_text
        elif re.match(r'[ウクスツヌフムユルグズヅブプヴイキシチニヒミリギジヂビピ]ェ', match_text):  # ウ段、イ段の後ろのェ
            return match_text
        else:
    ...
    replaced_text = re.sub(r'^[ァィゥェォヮャュョ]', replace_head, replaced_text, flags=re.MULTILINE)
    return replaced_text
...
def mora_split(text: str) -> list[str]:
    ...
    Returns:
        モーラ単位に分割されたリスト
    """
    pattern = r'[ウクスツヌフムユルグズヅブプ][ァヮィェォ]|[キシチニヒミリギジヂビピテデ][ャュョ]|[イキシチニヒミリギジヂビピ]ェ|[テデ]ィ|[トド]ゥ|[ァ-ヴー]'
...
class KanaToMora:
    ...
    def __init__(self):
        self.pattern = r'[ウクスツヌフムユルグズヅブプ][ァヮィェォ]|[キシチニヒミリギジヂビピテデ][ャュョ]|[イキシチニヒミリギジヂビピ]ェ|[テデ]ィ|[トド]ゥ|[ァ-ヴー]'
...
class KanaToSyllable:
    ...
    def __init__(self):
        ...
        re_multi_vowel = f"({re_multi_a}|{re_multi_i}|{re_multi_u}|{re_multi_e}|{re_multi_o})"
        re_multi_vowel += "(?![ーンッ])"
        ...
        re_single_vowel = f"({re_single_a}|{re_single_i}|{re_single_u}|{re_single_e}|{re_single_o})"
        # 1文字カナ単独のマッチ
        re_single_vowel += "(?![ーンッ])"
        ...
        # 上記で定義した条件のオアをとる
        self.re_all = re.compile(f"{re_multi_bar}|{re_multi_vowel}|{re_multi_unit}|{re_n_bar}|{re_single_bar}|{re_single_vowel}|{re_single_unit}")
        
        # ２文字以上で１シラブルの組み合わせ
        re_multi_kana_full = f"^({re_multi_bar}|{re_multi_vowel}|{re_multi_unit}|{re_n_bar}|{re_single_bar}|{re_single_vowel})$"
    ...
    def get_variation(self, syllables: list[str]) -> list[list[str]]:
        ...
        for syllable in syllables:
            ...
            elif syllable.endswith("ンッ"):  # ex: アンッ→["ア","ン","ッ"],["ア","ン"],["アー","ッ"],["アー"]
                head = syllable[:-2]
        ...
        for combination in all_combinations:
            # 空文字列を除去して平坦化
            flattened = [item for sublist in combination for item in sublist if item != ""]
...
def get_kana_to_vowel_dictionary(kana2phonon_dictionary: dict[str, list[str]]) -> dict[str, str]:
    """カナから母音への辞書を生成する.
...
def phonon_split(text: str) -> list[str]:
    ...
    re_single_unit = "[ァ-ヴー]"
    # 上記で定義した条件のオアをとる
    pattern = f"{re_multi_bar}|{re_multi_vowel}|{re_multi_unit}|{re_n_bar}|{re_single_bar}|{re_single_vowel}|{re_single_unit}"
...
class KanaConverter:
    ...
    def __init__(self, kana2phonon_data: Optional[dict[str, list[str]]] = None):
        """初期化.
        ...
        logger.info(f"KanaConverter initialized with {len(self.KANA2VOWEL_)} vowel mappings")
    ...
    def _get_default_kana2phonon(self) -> dict[str, list[str]]:
        ...
        return {
            "ア": ["", "a"], "イ": ["", "i"], "ウ": ["", "u"], "エ": ["", "e"], "オ": ["", "o"],
            "カ": ["k", "ka"], "キ": ["k", "ki"], "ク": ["k", "ku"], "ケ": ["k", "ke"], "コ": ["k", "ko"],
            "サ": ["s", "sa"], "シ": ["s", "si"], "ス": ["s", "su"], "セ": ["s", "se"], "ソ": ["s", "so"],
            "タ": ["t", "ta"], "チ": ["t", "ti"], "ツ": ["t", "tu"], "テ": ["t", "te"], "ト": ["t", "to"],
            "ナ": ["n", "na"], "ニ": ["n", "ni"], "ヌ": ["n", "nu"], "ネ": ["n", "ne"], "ノ": ["n", "no"],
            "ハ": ["h", "ha"], "ヒ": ["h", "hi"], "フ": ["h", "hu"], "ヘ": ["h", "he"], "ホ": ["h", "ho"],
            "マ": ["m", "ma"], "ミ": ["m", "mi"], "ム": ["m", "mu"], "メ": ["m", "me"], "モ": ["m", "mo"],
            "ヤ": ["y", "ya"], "ユ": ["y", "yu"], "ヨ": ["y", "yo"],
            "ラ": ["r", "ra"], "リ": ["r", "ri"], "ル": ["r", "ru"], "レ": ["r", "re"], "ロ": ["r", "ro"],
            "ワ": ["w", "wa"], "ヲ": ["w", "wo"],
            "ン": ["N", "N"], "ッ": ["q", "q"], "ー": ["", ""], "sp": ["sp", "sp"]
        }
    ...
    def _get_kana_units(self, kana2phonon: dict[str, list[str]], kana2vowel: dict[str, str]) -> dict[str, list[list[str]]]:
        """カナユニット辞書を生成."""
        result = {}
        for kana in kana2phonon.keys():
            ...
            if vowel_of_kana in ["ア", "イ", "ウ", "エ", "オ"]:
                ...
                result[kana + vowel_of_kana] = [[kana + "ー"], [kana, vowel_of_kana]]  # 母音の連続を伸ばし棒化
                if vowel_of_kana == "エ":
                    result[kana + "イ"] = [[kana + "ー"], [kana, "イ"]]  # eiを伸ばし棒化
                if vowel_of_kana == "オ":
                    result[kana + "ウ"] = [[kana + "ー"], [kana, "ウ"]]  # ouを伸ばし棒化
