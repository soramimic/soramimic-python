...
class TokenFormatter:
    ...
    def _remove_sign_pronunciation(self, tokens: list[dict[str, str]]) -> list[dict[str, str]]:
        # 記号: 発音を表層形に（JS コメントは「空文字にする」だが、実装は surface を入れている）
        for t in tokens:
            if t.get("pos") == "記号":
                t["pronunciation"] = t.get("surface_form", "")
        return tokens

    def _remove_unknown_sahen(self, tokens: list[dict[str, str]]) -> list[dict[str, str]]:
        # サ変接続で発音が未定義なら pos を記号に
        for t in tokens:
            if t.get("pos") == "名詞" and t.get("pos_detail_1") == "サ変接続" and not t.get("pronunciation"):
                t["pos"] = "記号"
                t["pronunciation"] = ""
        return tokens

    def _is_small_kana_start(self, ch: str) -> bool:
        return re.match(r'^[ぁぃぅぇぉゃゅょゎっァィゥェォヮャュョッ]', ch or "") is not None
    ...
    def _concat_small_kana(self, tokens: list[dict[str, Any]]) -> list[dict[str, str]]:
        ...
        for i in range(1, len(tokens)):
            if self._is_small_kana_start(tokens[i].get("surface_form", "")) and \
               self._is_kana_end(tokens[i-1].get("surface_form", "")):
                tokens[i-1]["surface_form"] = tokens[i-1].get("surface_form", "") + tokens[i].get("surface_form", "")
                tokens[i-1]["pronunciation"] = tokens[i-1].get("pronunciation", "") + tokens[i].get("pronunciation", "")
                tokens[i]["is_remove"] = True
        tokens = [v for v in tokens if not v.get("is_remove")]
        return tokens

    def _set_unknown_word_pronunciation(self, tokens: list[dict[str, str]]) -> list[dict[str, str]]:
        # 不明語（Kanaのみ）の読みを表層から設定
        for t in tokens:
            if t.get("word_type") == "UNKNOWN" and self._is_kana(t.get("surface_form", "")):
                t["pronunciation"] = self._hira_to_kata(t.get("surface_form", ""))
        return tokens

    def _set_kana_pronunciation(self, tokens: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    def _concat_single_bar(self, tokens: list[dict[str, str]]) -> list[dict[str, str]]:
        ...
        for i in range(1, len(tokens)):
            if tokens[i].get("surface_form") == "ー":
                tokens[i-1]["surface_form"] = tokens[i-1].get("surface_form", "") + "ー"
                tokens[i-1]["pronunciation"] = tokens[i-1].get("pronunciation", "") + "ー"
        tokens = [t for t in tokens if t.get("surface_form") != "ー"]
        return tokens

    
    def _set_number_pronunciation(self, tokens: list[dict[str, Any]]) -> list[dict[str, Any]]:
        n2p = {"1": "イチ", "2": "ニ", "3": "サン", "4": "ヨン", "5": "ゴ",
    ...
    def format(self, tokens: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        # tokens = self._set_unknown_word_pronunciation(tokens)  # 元コードではコメントアウト
...
class Kanji:
    ...
    def _kanji_allocate(self, surface: str, pronunciation: str, kanji_dict: dict[str, list[str]]) -> list[tuple[str, str]]:
...
class Character:
    ...
    def _kana_tokenize(self, text: str) -> list[dict[str, str]]:
        ...
        pattern = re.compile(r'(?P<kata>[ァ-ヴー]+)|(?P<hira>[ぁ-ゔー]+)|(?P<nonkana>[^ぁ-ゔァ-ヴー]+)')
        out = []
    ...
    def _balanced_allocate(self, surface: str, pronunciation: str) -> list[dict[str, Any]]:
        ...
        plusone = len(text[longer]) % len(text[shorter]) if len(text[shorter]) > 0 else 0
        contentlen = len(text[longer]) // len(text[shorter]) if len(text[shorter]) > 0 else 0
    ...
    def _kana_allocate(self, separated_surface: list[dict[str, str]], pronunciation: str) -> list[dict[str, Any]]:
        ...
        for i in range(len(separated_surface)):
            ...
            if start > 0:
                ...
                # JS の slice(0, start - len(rest_text)) は Python では rest_text[:start] と等価
                output.append({
    ...
    def _concat_sign_token(self, tokens: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for t in tokens:
            if t.get("type") == "sign":
                if formatted:
                    formatted[-1]["surface_form"] = formatted[-1].get("surface_form", "") + t.get("surface_form", "")
                continue
            else:
                formatted.append(t)
        return formatted

    def _get_char_correspondence(self, tokens: list[dict[str, Any]]) -> list[dict[str, Any]]:
        # token index 付与
        for idx, t in enumerate(tokens):
            t["token_index"] = idx
        ...
        for token in kana_correspondence:
            ...
            if ttype == "nonkana" and len(surf) > 1 and re.match(r"^[\w']+$", surf) is None:
                # 漢字（熟語）など
                ...
                for surface_form, yomi in pairs:
                    ...
                    if len(surface_form) > 1:
                        ...
                        for i in range(len(c)):
                            v = c[i]
                            if re.match(r'^[ァィゥェォヮャュョ]', v.get("pronunciation", "")) and \
                               v.get("in_surface_pos") == 0 and i > 0:
                                for j in range(i - 1, len(c)):
                                    if j >= i - 1 + 2 and c[j].get("in_surface_pos") == 0:
                                        break
